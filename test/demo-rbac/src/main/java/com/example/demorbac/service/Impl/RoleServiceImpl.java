...
import com.example.demorbac.entity.RoleModule;
...
public class RoleServiceImpl implements RoleService {
     ...
     */
    @Override
    public List<Role> selectAll(int pageNum) {
     ...
     */
    @Override
    public List<String> selectDynamic(Role role,int pageNum) {
        PageHelper.startPage(pageNum,pageSize);
    ...
    public Result addRole(Role role,Map map) {

        //账户名判重
        if(roleMapper.selectRoleNameRepeat(role.getRoleName())!=0){
            return new Result(false,"角色名重复");
        }
        ...
        if(roleDb==null){
            return new Result(false,"当前账户不具备任何角色，无法添加角色");
        }
        ...
        if(modules!=null) {
            for (Module module : modules) {
                ...
                roleModule.setModuleName(module.getModuleName());
    ...
    public Result updateRole(Role role, Map map) {
        ...
        if(role.getRoleName()!=null&&"超级管理员".equals(role.getRoleName())){
            return new Result(false,"不能更新为超级管理员");
        }
