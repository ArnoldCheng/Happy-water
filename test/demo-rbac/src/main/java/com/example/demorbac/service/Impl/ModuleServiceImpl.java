...
import com.example.demorbac.service.ModuleService;
...
public class ModuleServiceImpl implements ModuleService {
     ...
     */
    @Override
    public List<Module> selectAll(int pageNum) {
     ...
     */
    @Override
    public List<Module> selectDynamic(ModuleDynamic moduleDynamic,int pageNum) {
        PageHelper.startPage(pageNum,pageSize);
    ...
    public Result updateModule(Module module, Map map) {
        ...
        if(moduleMapper.selectModuleExistByModuleId(module.getId())==0){
            return new Result(false,"不存在该被编辑的模块");
        }
        ...
        if(fatherModuleId!=null){
            ...
            if (fatherModuleId == 0) {
                ...
                Integer fatherModuleLevel=moduleMapper.selectModuleLevelById(fatherModuleId);
                if(fatherModuleLevel>=3){
                    return new Result(false,"第三级模块不能更新子模块");
                }
                //并将更新的模块等级设置为父模块等级+1
                module.setModuleLevel(fatherModuleLevel+1);
            }
    ...
    public Result deleteModule(long id) {
        ...
        if(moduleLevel==1){
            if("后台管理".equals(module.getModuleName())){
                return new Result(false,"后台系统不可以被删除");
            }
            //获取子模块的id集合
    ...
    public List<ModuleJoin> selectModuleList(long managerId) {
        ...
        for(ModuleJoin moduleJoin:moduleJoinsOne){
            //设置一级模块的子模块属性
            moduleJoin.setFathermodule(moduleMapper.selectTopModuleById(managerId,moduleJoin.getId()));
        }
        //遍历一级模块
        for(ModuleJoin moduleJoin:moduleJoinsOne){
            //遍历二级权限的集合，根据账户ID和二级模块的ID获取三级权限的集合
            for(ModuleJoin moduleJoin1:moduleJoin.getFathermodule()){
                //设置二级模块的子模块属性
                moduleJoin1.setFathermodule(moduleMapper.selectTopModuleById(managerId,moduleJoin1.getId()));
            }
        }
        //返回封装了三级模块/二级模块的一级模块
