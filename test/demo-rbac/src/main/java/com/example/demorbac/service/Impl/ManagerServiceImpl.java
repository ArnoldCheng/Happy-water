...
import com.example.demorbac.entity.ManagerRole;
...
public class ManagerServiceImpl implements ManagerService {
     ...
     */
    @Override
    public List<Manager> selectAll(int pageNum) {
     ...
     */
    @Override
    public List<Manager> selectDynamic(ManagerDynamic managerDynamic, int pageNum) {
        PageHelper.startPage(pageNum,pageSize);
    ...
    public Result addManager(Manager manager,Map map){

        //账户判重，查询数据表中是否存在对应的账号名
        if(managerMapper.selectAccountRepeat(manager.getAccount())!=0){
            return new Result(false,"账户名重复",805);
        }
        ...
        //判断当前操作者是否具有角色
        if(roleDb==null){
            return new Result(false,"当前账户不具备任何角色");
        }
        ...
        if(roles!=null&&roles.size()!=0){
            ...
            for (Role role:roles){
                if("超级管理员".equals(role.getRoleName())){
                    return new Result(false,"不允许添加超级管理员");
                }
                //获取当前角色元素的角色等级
                int roleLevel=roleMapper.selectRoleLevelByRoleName(role.getRoleName());
                //如果当前操作者不是超级管理员且当前角色的等级不大于新添加的角色，则返回false
                if(!"超级管理员".equals(roleDb.getRoleName())
                        &&roleDb.getRoleLevel()>=roleLevel){
                    return new Result(false,"不允许添加高于等于自身级别的角色");
                }
        ...
        if(roles!=null&&roles.size()!=0) {
            ...
            for (Role role : roles) {
                ...
                //根据用户选择的角色名查询角色的id
                long id = roleMapper.selectRoleIdByRoleName(role.getRoleName());
                if (id == 0) {
                    return new Result(false, "账户已添加，但不存在该角色", 806);
                }
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public Result updateManager(Manager manager, Map map) {

        //判断被编辑的账户是否存在
        if(managerMapper.selectManagerExistByManagerId(manager.getId())==0){
            return new Result(false,"不存在账户");
        }

        //账户判重，查询数据表中是否存在对应的账号名
        if(managerMapper.selectAccountRepeat(manager.getAccount())!=0){
            return new Result(false,"账户名格式错误/重复，请重试。",805);
        }
        ...
        if(roles!=null&&roles.size()!=0){
            //遍历新添账户角色
            for (Role role:roles){
                loger.info("需要更新的角色名为： "+role.getRoleName());
                if("超级管理员".equals(role.getRoleName())){
                    return new Result(false,"不允许更新为超级管理员");
                }
                ...
                if(!"超级管理员".equals(roleDb.getRoleName())
                        &&roleDb.getRoleLevel()>=roleLevel){
                    return new Result(false,"不允许编辑高于等于自身级别的角色");
                }
        ...
        if(roles!=null&&roles.size()!=0) {
            for (Role role : roles) {
                ...
                managerRole.setRoleId(roleMapper.selectRoleIdByRoleName(role.getRoleName()));
                //设置更新时间
                managerRole.setUpdatedAt(System.currentTimeMillis());
    ...
    public Result deleteManager(long id,Map map) {
        ...
        if(managerMapper.selectManagerExistByManagerId(id)==0){
            return new Result(false,"不存在该账户");
        }
        ...
        //超级管理员不能被编辑
        if("超级管理员".equals(roleoriginal.getRoleName())){
            return new Result(false,"超级管理员不能被删除");
        }
